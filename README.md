...
Pi Agent Web runs a local Pi coding-agent session behind a loopback-only browser interface. The server serves the production web build and streams agent events directly to the local browser.
...
The gateway binds only to `127.0.0.1`. It is not designed or hardened for remote access, reverse proxies, shared machines, or untrusted local users. API and WebSocket access require loopback clients and local origins, but local mode intentionally has no authentication. Remote access will require a separate security design.
