    ...
    switch (event.type) {
      ...
      case 'ikanban:session-status':
        {
          ...
          if (sessionId && status) {
            ...
            if (status === 'busy') {
              updateSessionStatus(sessionId, { type: 'busy' }, 'sse:ikanban:session-status');
              ...
              }, 'sse:ikanban:session-status');
            } else {
              updateSessionStatus(sessionId, { type: 'idle' }, 'sse:ikanban:session-status');
            }
      ...
      case 'question.asked': {
        ...
        const toastKey = `${request.sessionID}:${request.id}`;
	        ...
	        // notifications are emitted server-side (see ikanban:notification)
