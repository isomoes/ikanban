...
export function ThemeSystemProvider({ children, defaultThemeId }: ThemeSystemProviderProps) {
  ...
  const [vscodeTheme, setVSCodeTheme] = useState<Theme | null>(() => {
    ...
    const existing = (window as unknown as { __IKANBAN_VSCODE_THEME__?: Theme }).__IKANBAN_VSCODE_THEME__;
  ...
  useEffect(() => {
    ...
    window.addEventListener('ikanban:vscode-theme', handleThemeEvent as EventListener);
    return () => window.removeEventListener('ikanban:vscode-theme', handleThemeEvent as EventListener);
  }, [isVSCode]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('ikanban:settings-synced', handleSettingsSynced);
    return () => window.removeEventListener('ikanban:settings-synced', handleSettingsSynced);
  }, []);
