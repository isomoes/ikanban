...
const getVSCodeWorkspaceProject = (): { projects: ProjectEntry[]; activeProjectId: string | null } | null => {
  ...
  const runtimeApis = (window as unknown as { __IKANBAN_RUNTIME_APIS__?: { runtime?: { isVSCode?: boolean } } })
    .__IKANBAN_RUNTIME_APIS__;
  ...
  if (streamDebugEnabled()) {
    console.log('[IKanban][VSCode][projects] Using workspace fallback project', entry);
  }
...
if (typeof window !== 'undefined') {
  window.addEventListener('ikanban:settings-synced', (event: Event) => {
