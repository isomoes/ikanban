...
const MODELS_DEV_PROXY_URL = "/api/ikanban/models-metadata";

const FALLBACK_PROVIDER_ID = "opencode";
const FALLBACK_MODEL_ID = "big-pickle";

interface IKanbanDefaults {
    defaultModel?: string;
...
const fetchIKanbanDefaults = async (): Promise<IKanbanDefaults> => {
    try {
...
interface ConfigStore {
    ...
    // IKanban settings-based defaults (take precedence over agent preferences)
        ...
        persist(
            (set, get) => ({
                ...
                loadAgents: async (options) => {
                    ...
                    for (let attempt = 0; attempt < 3; attempt++) {
                        try {
                            // Fetch agents and IKanban settings in parallel
                            const [agents, iKanbanDefaults] = await Promise.all([
                                opencodeClient.withDirectory(fromDirectoryKey(directoryKey), () => opencodeClient.listAgents()),
                                fetchIKanbanDefaults(),
                            ]);
                            ...
                            set((state) => {
                                ...
                                const nextState: Partial<ConfigStore> = {
                                    settingsDefaultModel: iKanbanDefaults.defaultModel,
                                    settingsDefaultVariant: iKanbanDefaults.defaultVariant,
                                    settingsDefaultAgent: iKanbanDefaults.defaultAgent,
                                    settingsAutoCreateWorktree: iKanbanDefaults.autoCreateWorktree ?? false,
                                    settingsGitmojiEnabled: iKanbanDefaults.gitmojiEnabled ?? false,
                                    settingsZenModel: iKanbanDefaults.zenModel,
                                    directoryScoped: {
                                        ...state.directoryScoped,
                                        [directoryKey]: nextSnapshot,
                                    },
                            ...
                            // 1. Check IKanban settings for default agent
                            if (iKanbanDefaults.defaultAgent) {
                                const settingsAgent = safeAgents.find((agent) => agent.name === iKanbanDefaults.defaultAgent);
                                if (settingsAgent) {
                             ...
                             // 1. Check IKanban settings for default model
                             if (iKanbanDefaults.defaultModel) {
                                 const parsed = parseModelString(iKanbanDefaults.defaultModel);
                                 if (parsed && validateModel(parsed.providerId, parsed.modelId)) {
                                     ...
                                     if (iKanbanDefaults.defaultVariant) {
                                         const provider = providers.find((p) => p.id === parsed.providerId);
                                         const model = provider?.models.find((m) => m.id === parsed.modelId) as { variants?: Record<string, unknown> } | undefined;
                                         const variants = model?.variants;
                                         if (variants && Object.prototype.hasOwnProperty.call(variants, iKanbanDefaults.defaultVariant)) {
                                             resolvedVariant = iKanbanDefaults.defaultVariant;
                                         } else {
                ...
                setAgent: (agentName: string | undefined) => {
                    ...
                    if (agentName && typeof window !== "undefined") {
                        ...
                        if (sessionStore) {
                            ...
                            const { currentSessionId, isIKanbanCreatedSession, initializeNewIKanbanSession, getAgentModelForSession } = sessionState;
                            ...
                            if (currentSessionId && isIKanbanCreatedSession(currentSessionId)) {
                                const existingAgentModel = getAgentModelForSession(currentSessionId, agentName);
                                if (!existingAgentModel) {

                                    initializeNewIKanbanSession(currentSessionId, agents);
                                }
                            }
                        }
                    }
