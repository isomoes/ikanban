    ...
    devtools(
        (set, get) => ({
            ...
            setVoiceMode: (mode: import("./types/sessionTypes").VoiceMode) => {
                    ...
                    if (draft?.open) {
                        ...
                        try {
                            useSessionManagementStore
                                .getState()
                                .initializeNewIKanbanSession(created.id, configState.agents);
                        } catch {
                            // ignored
                        }
                ...
                isIKanbanCreatedSession: (sessionId: string) => useSessionManagementStore.getState().isIKanbanCreatedSession(sessionId),
                markSessionAsIKanbanCreated: (sessionId: string) => useSessionManagementStore.getState().markSessionAsIKanbanCreated(sessionId),
                initializeNewIKanbanSession: (sessionId: string, agents: Record<string, unknown>[]) => useSessionManagementStore.getState().initializeNewIKanbanSession(sessionId, agents),
