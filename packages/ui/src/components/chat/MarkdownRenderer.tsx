...
const useMarkdownShikiThemes = (): readonly [string | object, string | object] => {
  ...
  const getThemes = React.useCallback((): readonly [string | object, string | object] => {
    ...
    const provided = window.__IKANBAN_VSCODE_SHIKI_THEMES__;
  ...
  React.useEffect(() => {
    ...
    const handler = (event: Event) => {
      // Rely on the canonical `window.__IKANBAN_VSCODE_SHIKI_THEMES__` that the webview updates
      // before dispatching this event, so we always apply stable cache keys and avoid stale token reuse.
      void event;
      setThemes(getThemes());
    };

    window.addEventListener('ikanban:vscode-shiki-themes', handler as EventListener);
    return () => window.removeEventListener('ikanban:vscode-shiki-themes', handler as EventListener);
  }, [getThemes, isVSCode]);
