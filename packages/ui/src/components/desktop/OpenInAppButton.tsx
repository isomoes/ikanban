...
export const OpenInAppButton = ({ directory, className }: OpenInAppButtonProps) => {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ikanban:settings-synced', handler);
    return () => window.removeEventListener('ikanban:settings-synced', handler);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ikanban:app-ready', handler);
    ...
    window.addEventListener('ikanban:installed-apps-updated', updateHandler);
    const flag = (window as unknown as { __ikanbanAppReady?: boolean }).__ikanbanAppReady;
    if (flag) {
      void loadInstalledApps();
    }
    return () => {
      window.removeEventListener('ikanban:app-ready', handler);
      window.removeEventListener('ikanban:installed-apps-updated', updateHandler);
    };
