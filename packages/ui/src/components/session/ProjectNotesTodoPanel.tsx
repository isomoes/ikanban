...
import {
  ...
  IKANBAN_PROJECT_NOTES_MAX_LENGTH,
  IKANBAN_PROJECT_TODO_TEXT_MAX_LENGTH,
  saveProjectNotesAndTodos,
  type IKanbanProjectTodoItem,
  type ProjectRef,
} from '@/lib/ikanbanConfig';
...
export const ProjectNotesTodoPanel: React.FC<ProjectNotesTodoPanelProps> = ({
  ...
  const [todos, setTodos] = React.useState<IKanbanProjectTodoItem[]>([]);
  ...
  const persistProjectData = React.useCallback(
    async (nextNotes: string, nextTodos: IKanbanProjectTodoItem[]) => {
      if (!projectRef) {
        return false;
      }
  ...
  const handleAddTodo = React.useCallback(() => {
    ...
    const nextTodos = [
      ...
      {
        id: createTodoId(),
        text: trimmed.slice(0, IKANBAN_PROJECT_TODO_TEXT_MAX_LENGTH),
        completed: false,
        createdAt: Date.now(),
      },
    ];
  ...
  const todoInputValue = newTodoText.slice(0, IKANBAN_PROJECT_TODO_TEXT_MAX_LENGTH);
    ...
    <div className={cn('w-full min-w-0 space-y-3 p-3', className)}>
      <div className="space-y-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="typography-meta text-muted-foreground">{notes.length}/{IKANBAN_PROJECT_NOTES_MAX_LENGTH}</span>
        </div>
        <Textarea
          value={notes}
          onChange={(event) => setNotes(event.target.value.slice(0, IKANBAN_PROJECT_NOTES_MAX_LENGTH))}
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-1.5">
          <Input
            ...
            onChange={(event) => setNewTodoText(event.target.value.slice(0, IKANBAN_PROJECT_TODO_TEXT_MAX_LENGTH))}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
