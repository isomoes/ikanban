...
export const AgentManagerView: React.FC<AgentManagerViewProps> = ({ className }) => {
  const isVSCodeRuntime = Boolean(
    (typeof window !== 'undefined'
      ? (window as unknown as { __IKANBAN_RUNTIME_APIS__?: { runtime?: { isVSCode?: boolean } } })
          .__IKANBAN_RUNTIME_APIS__?.runtime?.isVSCode
      : false)
  );
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () =>
      (typeof window !== 'undefined'
        ? (window as unknown as { __IKANBAN_CONNECTION__?: { status?: string } }).__IKANBAN_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as unknown as { __IKANBAN_CONNECTION__?: { status?: string } }).__IKANBAN_CONNECTION__?.status
    ...
    const runBootstrap = async () => {
      try {
        ...
        if (streamDebugEnabled()) {
          console.log('[IKanban][VSCode][agentManager] bootstrap complete', {
