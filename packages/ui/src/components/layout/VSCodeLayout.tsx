...
export const VSCodeLayout: React.FC = () => {
  ...
  const [connectionStatus, setConnectionStatus] = React.useState<'connecting' | 'connected' | 'error' | 'disconnected'>(
    () => (typeof window !== 'undefined'
      ? (window as { __IKANBAN_CONNECTION__?: { status?: string } }).__IKANBAN_CONNECTION__?.status as
  ...
  React.useEffect(() => {
    ...
    void vscodeApi.executeCommand('ikanban.setActiveSession', currentSessionId, activeSessionTitle);
  ...
  React.useEffect(() => {
    ...
    const current =
      (typeof window !== 'undefined'
        ? (window as { __IKANBAN_CONNECTION__?: { status?: string } }).__IKANBAN_CONNECTION__?.status
    ...
    window.addEventListener('ikanban:connection-status', handler as EventListener);
    return () => window.removeEventListener('ikanban:connection-status', handler as EventListener);
  }, []);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('ikanban:navigate', handler as EventListener);
    return () => window.removeEventListener('ikanban:navigate', handler as EventListener);
  }, []);
    ...
    const runBootstrap = async () => {
      ...
      try {
        const debugEnabled = (() => {
          ...
          try {
            return window.localStorage.getItem('ikanban_stream_debug') === '1';
          } catch {
            return false;
          }
        })();

        if (debugEnabled) console.log('[IKanban][VSCode][bootstrap] attempt', { configInitialized });
        ...
        if (debugEnabled) console.log('[IKanban][VSCode][bootstrap] post-load', {
