...
const resolveDesktopBaseUrl = (): string | null => {
  ...
  const desktopServer = (window as typeof window & {
    __IKANBAN_DESKTOP_SERVER__?: { origin: string; apiPrefix?: string };
    __IKANBAN_RUNTIME_APIS__?: RuntimeAPIs;
  }).__IKANBAN_DESKTOP_SERVER__;

  const isDesktop = Boolean(
    (window as typeof window & { __IKANBAN_RUNTIME_APIS__?: RuntimeAPIs }).__IKANBAN_RUNTIME_APIS__?.runtime?.isDesktop
  );
...
class OpencodeService {
  ...
  private getGlobalSseCoalesceKey(event: RoutedOpencodeEvent): string | null {
    ...
    if (eventType === 'ikanban:session-status') {
      const sessionId = typeof properties?.sessionId === 'string'
        ? properties.sessionId
        : typeof properties?.sessionID === 'string'
          ? properties.sessionID
      ...
      return `ikanban:session-status:${sessionId}`;
    }
  ...
  async checkHealth(): Promise<boolean> {
    try {
      ...
      // Check if the upstream API is ready (not just IKanban server)
