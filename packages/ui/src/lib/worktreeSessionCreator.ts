...
import { getWorktreeSetupCommands } from '@/lib/ikanbanConfig';
import {
...
export async function createWorktreeSession(): Promise<{ id: string } | null> {
  ...
  try {
    ...
    sessionStore.initializeNewIKanbanSession(session.id, agents);
...
export async function createWorktreeSessionForBranch(
  ...
  try {
    ...
    if (!projectRef) {
      throw new Error('Project is not registered in IKanban');
    }
  ...
  try {
    ...
    try {
      ...
      const createdMetadata = {
        ...
        const configState = useConfigStore.getState();
        sessionStore.initializeNewIKanbanSession(session.id, configState.agents);
        sessionStore.setSessionDirectory(session.id, metadata.path);
        sessionStore.setWorktreeMetadata(session.id, createdMetadataWithStatus);
