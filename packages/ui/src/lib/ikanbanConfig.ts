/**
 * IKanban project-level configuration service.
 * Stores per-project settings in ~/.config/ikanban/<projectId>.json.
 * Migrates from legacy <project>/.ikanban/ikanban.json.
 */
...
const CONFIG_FILENAME = 'ikanban.json';
// LEGACY_PROJECT_CONFIG: legacy per-project config root inside repo.
const LEGACY_CONFIG_DIR = '.ikanban';
const USER_CONFIG_DIR_SEGMENTS = ['.config', 'ikanban'];
const USER_PROJECTS_DIR_SEGMENTS = ['.config', 'ikanban', 'projects'];
...
function getRuntimeFilesAPI(): FilesAPI | null {
  ...
  const apis = (window as typeof window & { __IKANBAN_RUNTIME_APIS__?: RuntimeAPIs }).__IKANBAN_RUNTIME_APIS__;
  if (apis?.files) {
    return apis.files;
  }
  return null;
...
export interface IKanbanConfig {
  ...
  projectNotes?: string;
  projectTodos?: IKanbanProjectTodoItem[];
}

export interface IKanbanProjectTodoItem {
  id: string;
  text: string;
  completed: boolean;
  createdAt: number;
...
export interface IKanbanProjectNotesTodos {
  notes: string;
  todos: IKanbanProjectTodoItem[];
}

export const IKANBAN_PROJECT_NOTES_MAX_LENGTH = 1000;
export const IKANBAN_PROJECT_TODO_TEXT_MAX_LENGTH = 120;
...
const resolveHomeDirectory = async (): Promise<string | null> => {
  // VSCode webview sets __IKANBAN_HOME__ to workspace folder (not OS home).
  // For user config (~/.config/ikanban), always use /api/fs/home in VSCode.
  if (!isVSCodeRuntime()) {
...
const sanitizeProjectNotes = (value: unknown): string => {
  ...
  return trimToMaxLength(value, IKANBAN_PROJECT_NOTES_MAX_LENGTH);
};

const sanitizeProjectTodoItems = (value: unknown): IKanbanProjectTodoItem[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const sanitized: IKanbanProjectTodoItem[] = [];
  for (const entry of value) {
    ...
    const text = trimToMaxLength(textRaw.trim(), IKANBAN_PROJECT_TODO_TEXT_MAX_LENGTH);
    if (!id || !text) {
      continue;
    }
...
const sanitizeProjectNotesAndTodos = (value: {
  notes?: unknown;
  todos?: unknown;
} | null | undefined): IKanbanProjectNotesTodos => {
  return {
 ...
 */
export async function readIKanbanConfig(project: ProjectRef): Promise<IKanbanConfig | null> {
  const projectDirectory = typeof project?.path === 'string' ? project.path.trim() : '';
  if (!projectDirectory) {
    return null;
  }
  ...
  const parseConfig = (text: string | null): IKanbanConfig | null => {
    ...
    try {
      ...
      return parsed as IKanbanConfig;
    } catch {
      return null;
    }
  };
  ...
  // 2) Migrate legacy <project>/.ikanban/ikanban.json.
  // LEGACY_PROJECT_CONFIG: migrate project-local ikanban.json -> ~/.config/ikanban/projects/<projectId>.json
  ...
  try {
    const wrote = await writeIKanbanConfig(project, legacyConfig);
    if (wrote) {
      await deleteLegacyIKanbanConfig(projectDirectory);
    }
  } catch {
 ...
 */
export async function writeIKanbanConfig(
  project: ProjectRef,
  ...
  try {
    ...
    console.error('Failed to write ikanban config:', error);
    return false;
  }
}
 ...
 */
export async function updateIKanbanConfig(
  project: ProjectRef,
  updates: Partial<IKanbanConfig>
): Promise<boolean> {
  const existing = await readIKanbanConfig(project) || {};
  const merged = { ...existing, ...updates };
  return writeIKanbanConfig(project, merged);
}
...
export async function getWorktreeSetupCommands(project: ProjectRef): Promise<string[]> {
  const config = await readIKanbanConfig(project);
...
export async function saveWorktreeSetupCommands(project: ProjectRef, commands: string[]): Promise<boolean> {
  ...
  return updateIKanbanConfig(project, { 'setup-worktree': filtered });
}

export async function getProjectNotesAndTodos(project: ProjectRef): Promise<IKanbanProjectNotesTodos> {
  const config = await readIKanbanConfig(project);
...
export async function saveProjectNotesAndTodos(
  project: ProjectRef,
  value: IKanbanProjectNotesTodos
): Promise<boolean> {
  ...
  return updateIKanbanConfig(project, {
...
async function deleteLegacyIKanbanConfig(projectDirectory: string): Promise<void> {
  const legacyPath = getLegacyConfigPath(projectDirectory);
  const runtimeFiles = getRuntimeFilesAPI();
