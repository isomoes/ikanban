...
const DESKTOP_NOTIFY_PREFIX = '[IKanbanDesktopNotify] ';
...
const IKANBAN_USER_CONFIG_ROOT = path.join(os.homedir(), '.config', 'ikanban');
const IKANBAN_USER_THEMES_DIR = path.join(IKANBAN_USER_CONFIG_ROOT, 'themes');
...
const readCustomThemesFromDisk = async () => {
  try {
    const entries = await fsPromises.readdir(IKANBAN_USER_THEMES_DIR, { withFileTypes: true });
    const themes = [];
    const seen = new Set();
    ...
    for (const entry of entries) {
      ...
      const filePath = path.join(IKANBAN_USER_THEMES_DIR, entry.name);
      try {
...
const resolveWorkspacePath = (targetPath, baseDirectory) => {
  ...
  // Allow writing IKanban per-project config under ~/.config/ikanban.
  // LEGACY_PROJECT_CONFIG: migration target root; allowed outside workspace.
  if (isPathWithinRoot(resolved, IKANBAN_USER_CONFIG_ROOT)) {
    return { ok: true, base: path.resolve(IKANBAN_USER_CONFIG_ROOT), resolved };
  }
...
const IKANBAN_DATA_DIR = process.env.IKANBAN_DATA_DIR
  ? path.resolve(process.env.IKANBAN_DATA_DIR)
  : path.join(os.homedir(), '.config', 'ikanban');
const SETTINGS_FILE_PATH = path.join(IKANBAN_DATA_DIR, 'settings.json');
const PUSH_SUBSCRIPTIONS_FILE_PATH = path.join(IKANBAN_DATA_DIR, 'push-subscriptions.json');
...
const resolveVapidSubject = async () => {
  const configured = process.env.IKANBAN_VAPID_SUBJECT;
  if (typeof configured === 'string' && configured.trim().length > 0) {
    return configured.trim();
  }

  const originEnv = process.env.IKANBAN_PUBLIC_ORIGIN;
  if (typeof originEnv === 'string' && originEnv.trim().length > 0) {
    ...
    if (trimmed.startsWith('http://localhost')) {
      return 'mailto:ikanban@localhost';
    }
    return trimmed;
  }
...
const ensurePushInitialized = async () => {
  ...
  if (subject === 'mailto:ikanban@localhost') {
    console.warn('[Push] No public origin configured for VAPID; set IKANBAN_VAPID_SUBJECT or enable push once from a real origin.');
  }
...
const HMR_STATE_KEY = '__ikanbanHmrState';
const getHmrState = () => {
...
const ENV_CONFIGURED_OPENCODE_PORT = (() => {
  const raw =
    ...
    process.env.IKANBAN_OPENCODE_PORT ||
    process.env.IKANBAN_INTERNAL_PORT;
  if (!raw) {
    return null;
  }
...
const ENV_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START === 'true' ||
                                    process.env.IKANBAN_SKIP_OPENCODE_START === 'true';
const ENV_DESKTOP_NOTIFY = process.env.IKANBAN_DESKTOP_NOTIFY === 'true';
...
function resolveOpencodeCliPath() {
  const explicit = [
    ...
    process.env.IKANBAN_OPENCODE_PATH,
    process.env.IKANBAN_OPENCODE_BIN,
  ]
...
function resolveNodeCliPath() {
  const explicit = [process.env.NODE_BINARY, process.env.IKANBAN_NODE_BINARY]
    .map((v) => (typeof v === 'string' ? v.trim() : ''))
    .filter(Boolean);
...
function resolveBunCliPath() {
  const explicit = [process.env.BUN_BINARY, process.env.IKANBAN_BUN_BINARY]
    .map((v) => (typeof v === 'string' ? v.trim() : ''))
    .filter(Boolean);
...
function parseArgs(argv = process.argv.slice(2)) {
  ...
  const envPassword =
    process.env.IKANBAN_UI_PASSWORD ||
    process.env.OPENCODE_UI_PASSWORD ||
    null;
  const envCfTunnel = process.env.IKANBAN_TRY_CF_TUNNEL === 'true';
...
async function main(options = {}) {
  ...
  console.log(`Starting IKanban on port ${port === 0 ? 'auto' : port}`);
  ...
  app.get('/api/ikanban/update-check', async (_req, res) => {
    try {
  ...
  app.post('/api/ikanban/update-install', async (_req, res) => {
    try {
      ...
      const instanceFilePath = path.join(tmpDir, `ikanban-${currentPort}.json`);
      ...
      let restartCmd = `ikanban serve --port ${storedOptions.port} --daemon`;
      ...
      setTimeout(() => {
        ...
        const script = isWindows
          ? `
            ...
            ${updateCmd}
            if %ERRORLEVEL% EQU 0 (
              echo Update successful, restarting IKanban...
              ${restartCmd}
            ) else (
              echo Update failed
              exit /b 1
  ...
  app.get('/api/ikanban/models-metadata', async (req, res) => {
    const now = Date.now();
  ...
  app.get('/api/global/event', async (req, res) => {
    ...
    const heartbeatInterval = setInterval(() => {
      writeSseEvent(res, { type: 'ikanban:heartbeat', timestamp: Date.now() });
    }, 15000);
...
`);
  ...
  // Note: scopes may be overridden via IKANBAN_GITHUB_SCOPES or settings.json (see github-auth.js).
  ...
  app.post('/api/github/auth/start', async (_req, res) => {
    try {
      ...
      if (!clientId) {
        return res.status(400).json({
          error: 'GitHub OAuth client not configured. Set IKANBAN_GITHUB_CLIENT_ID.',
        });
      }
...
- head branch: ${head}`;
  ...
  app.get('/api/git/worktrees', async (req, res) => {
    ...
    try {
      ...
      res.setHeader('X-IKanban-Warning', 'git worktrees unavailable');
      res.json([]);
    }
  });
  ...
  const COMMAND_TIMEOUT_MS = (() => {
    const raw = Number(process.env.IKANBAN_FS_EXEC_TIMEOUT_MS);
  ...
  const distPath = (() => {
    const env = typeof process.env.IKANBAN_DIST_DIR === 'string' ? process.env.IKANBAN_DIST_DIR.trim() : '';
    if (env) {
  ...
  const bindHost = typeof process.env.IKANBAN_HOST === 'string' && process.env.IKANBAN_HOST.trim().length > 0
    ? process.env.IKANBAN_HOST.trim()
    : null;
  ...
  await new Promise((resolve, reject) => {
    ...
    const onListening = async () => {
      ...
      try {
        process.send?.({ type: 'ikanban:ready', port: activePort });
      } catch {
        // ignore
      }

      console.log(`IKanban server running on port ${activePort}`);
