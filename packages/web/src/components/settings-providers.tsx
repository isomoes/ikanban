...
export const SettingsProviders: Component = () => {
  ...
  const isConfigCustom = (providerID: string) => {
    const provider = globalSync.data.config.providers?.[providerID]
    if (!provider) return false
    if (provider.package !== "aisdk:@ai-sdk/openai-compatible") return false
    if (!provider.models || Object.keys(provider.models).length === 0) return false
    return true
  }

  const disableProvider = async (providerID: string, name: string) => {
    const before = globalSync.data.config.experimental?.policies ?? []
    const next = [...before, { action: "provider.use" as const, resource: providerID, effect: "deny" as const }]

    await globalSync
      .updateConfig({ experimental: { policies: next } })
      .then(() => {
        showToast({
          variant: "success",
          icon: "circle-check",
