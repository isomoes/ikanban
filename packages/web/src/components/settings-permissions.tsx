...
export const SettingsPermissions: Component = () => {
  ...
  const permission = createMemo(() => globalSync.data.config.permissions ?? [])

  const actionFor = (id: string): PermissionAction => {
    return permission().findLast((rule) => (rule.action === id || rule.action === "*") && rule.resource === "*")?.effect ?? "allow"
  }

  const setPermission = async (id: string, action: PermissionAction) => {
    const before = globalSync.data.config.permissions
    const rules = (before ?? []).filter((rule) => rule.action !== id || rule.resource !== "*")
    const index = rules.findIndex((rule) => rule.action === id && rule.resource !== "*")
    rules.splice(index < 0 ? rules.length : index, 0, { action: id, resource: "*", effect: action })

    const rollback = (err: unknown) => {
      globalSync.set("config", "permissions", before)
      const message = err instanceof Error ? err.message : String(err)
      showToast({ title: language.t("settings.permissions.toast.updateFailed.title"), description: message })
    }

    globalSync.set("config", "permissions", rules)
    globalSync
      .updateConfig({ permissions: rules })
      .catch(rollback)
  }
      ...
      <div class="flex flex-col gap-6 px-4 py-6 sm:p-8 sm:pt-6 max-w-[720px]">
        <div class="flex flex-col gap-2">
          ...
          <div class="border border-border-weak-base rounded-lg overflow-hidden">
            <For each={ITEMS.filter((item) => !["lsp", "todoread", "codesearch"].includes(item.id))}>
              {(item) => (
