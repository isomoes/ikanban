...
export function DialogCustomProvider(props: Props) {
  ...
  const validate = () => {
    const output = validateCustomProvider({
      form,
      t: language.t,
      disabledProviders: (globalSync.data.config.experimental?.policies ?? []).filter((rule) => rule.action === "provider.use" && rule.effect === "deny").map((rule) => rule.resource),
  ...
  const save = async (e: SubmitEvent) => {
    ...
    const policies = (globalSync.data.config.experimental?.policies ?? []).filter((rule) => rule.action !== "provider.use" || rule.resource !== result.providerID)
    globalSync.updateConfig({ providers: { [result.providerID]: result.config }, experimental: { policies } })
      .then(() => {
        dialog.close()
        showToast({
          variant: "success",
