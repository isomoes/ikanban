...
function useServerPreview(fetcher: typeof fetch) {
  ...
  const previewStatus = async (
    ...
    const http: ServerConnection.HttpBase = { url: normalized, authType: username ? "basic" : "bearer" }
...
export function DialogSelectServer() {
  ...
  async function handleAdd(value: string) {
    ...
    const conn: ServerConnection.Http = {
      type: "http",
      http: { url: normalized, authType: store.addServer.username ? "basic" : "bearer" },
    }
  ...
  async function handleEdit(original: ServerConnection.Any, value: string) {
    ...
    const conn: ServerConnection.Http = {
      type: "http",
      displayName: name,
      http: { url: normalized, username, password, authType: username ? "basic" : "bearer" },
    }
    const result = await checkServerHealth(conn.http, fetcher)
  ...
  const startEdit = (conn: ServerConnection.Http) => {
    ...
    setStore("editServer", {
      ...
      username: conn.http.username ?? (conn.http.password && conn.http.authType !== "bearer" ? "opencode" : ""),
