...
import { appHref } from "@/shell/routes/base"
...
test("classifies deployed home, settings, draft, and server session deep links", () => {
  const server = ServerConnection.Key.make("https://backend.example:4096")
  expect(currentRoute(appHref("/"), "")).toEqual({ type: "home" })
  expect(currentRoute(appHref("/settings"), "")).toEqual({ type: "settings" })
  expect(currentRoute(appHref("/new-session"), "?draftId=a%2Fb")).toEqual({ type: "draft", draftID: "a/b" })
  expect(currentRoute(appHref(sessionHref(server, "session-1")), "?view=files")).toEqual({
    type: "session",
    sessionId: "session-1",
    server,
  })
  expect(() => currentRoute("/ikanban-other/settings", "")).toThrow("Unrecognised route")
})
