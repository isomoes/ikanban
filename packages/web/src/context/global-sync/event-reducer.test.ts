...
import type { Message, Part, PermissionRequest, Project, QuestionRequest, Session } from "@/client/types"
...
test("native message snapshots replace optimistic and removed parts instead of duplicating the prompt", () => {
  const [store, setStore] = createStore(baseState({
    part: { msg_1: [textPart("prt_optimistic", "ses_1", "msg_1"), textPart("prt_stale", "ses_1", "msg_1")] },
  }))
  const nativePart = textPart("msg_1:0000", "ses_1", "msg_1")
  applyDirectoryEvent({
    event: { type: "message.parts.updated", properties: { messageID: "msg_1", parts: [nativePart] } },
    store, setStore, directory: "/tmp", push: () => {}, loadLsp: () => {},
  })
  expect(store.part.msg_1).toEqual([nativePart])
})
