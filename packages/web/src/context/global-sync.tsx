    ...
    if (directory === "global") {
      ...
      if (event.type === "server.connected" || event.type === "global.disposed") {
        for (const directory of Object.keys(children.children)) {
          ...
          // V2 subscriptions have no replay. Reconcile loaded timelines after reconnect.
          for (const sessionID of Object.keys(store.message)) {
            void sdkFor(directory).session.messages({ sessionID }).then((result) => {
              if (children.children[directory]?.[0] !== store) return
              batch(() => {
                setStore("message", sessionID, reconcile(result.data.map((message) => message.info), { key: "id" }))
                for (const message of result.data) setStore("part", message.info.id, reconcile(message.parts, { key: "id" }))
              })
            }).catch((error) => console.error("Failed to resync session after reconnect", error))
          }
        }
      }
      return
