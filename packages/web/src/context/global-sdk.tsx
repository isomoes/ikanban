import type { GlobalEvent } from "@/client/types"
...
export const { use: useGlobalSDK, provider: GlobalSDKProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const key = (directory: string, payload: Event) => {
      ...
      if (payload.type === "message.parts.updated") return `message.parts.updated:${directory}:${payload.properties.messageID}`
    ...
    const resetHeartbeat = () => {
      ...
      // V2 keepalives are consumed by the client and are not application events.
      // A quiet session must not be treated as a dead connection.
    }
