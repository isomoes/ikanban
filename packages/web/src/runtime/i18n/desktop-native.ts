...
export function detectDesktopNativeLocale(languages: readonly string[]): DesktopNativeLocale {
  for (const language of languages) {
    ...
    if (!source) continue
    const match = DESKTOP_NATIVE_LOCALES.find(
      (candidate) => locale(DESKTOP_NATIVE_LOCALE_TAGS[candidate])?.language === source.language,
    )
...
export const DESKTOP_NATIVE_ENGLISH = {
  ...
  "desktop.menu.documentation": "iKanban Documentation",
  "desktop.menu.supportForum": "GitHub Issues",
