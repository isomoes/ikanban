...
const GLOBAL_STORAGE = "ikanban.v2.global.dat"
const WINDOW_STORAGE = "ikanban.v2.window"
const DIRECT_STORAGE = "ikanban.v2.direct.dat"
const LOCAL_PREFIX = "ikanban.v2."
...
function workspaceStorage(dir: string) {
  ...
  return `${LOCAL_PREFIX}workspace.${head}.${sum}.dat`
}
...
function draftStorage(draftID: string) {
  ...
  return `${LOCAL_PREFIX}draft.${head}.${sum}.dat`
}
...
function localStorageDirect(): SyncStorage {
  // Bare targets and previousKey relocation stay inside this app's namespace.
  return localStorageWithPrefix(DIRECT_STORAGE)
}
...
export function persisted<S extends Schema.ConstraintCodec<object, unknown>>(
  ...
  const channel =
    config.sync && typeof BroadcastChannel !== "undefined"
      ? new BroadcastChannel(`ikanban.v2.persist:${config.storage ?? DIRECT_STORAGE}:${config.key}`)
      : undefined
