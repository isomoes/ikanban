...
describe("prompt persistence", () => {
  ...
  test("relocates a previous key into canonical storage", () => {
    localStorage.setItem("ikanban.v2.direct.dat:server.v3", JSON.stringify({ list: ["https://example.com"] }))
    ...
    expect(localStorage.getItem("ikanban.v2.global.dat:server")).toBe(JSON.stringify({ list: ["https://example.com"] }))
  ...
  test("relocates a previous prompt key into the draft store", async () => {
    ...
    const oldKey = `ikanban.v2.direct.dat:${dir}/prompt.v2`
