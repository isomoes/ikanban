...
function parseArgs() {
  ...
  const envPassword = process.env.IKANBAN_UI_PASSWORD || undefined;
...
ENVIRONMENT:
  IKANBAN_UI_PASSWORD      Alternative to --ui-password flag
...
async function getPidFilePath(port) {
  ...
  return path.join(tmpDir, `ikanban-${port}.pid`);
}

async function getInstanceFilePath(port) {
  const os = await import('os');
  const tmpDir = os.tmpdir();
  return path.join(tmpDir, `ikanban-${port}.json`);
}
...
const commands = {
  async serve(options) {
    ...
    if (options.port !== 0) {
      ...
      if (existingPid && isProcessRunning(existingPid)) {
        console.error(`Error: IKanban is already running on port ${options.port} (PID: ${existingPid})`);
        console.error('Use "ikanban stop" to stop the existing instance');
        process.exit(1);
      }
    ...
    if (options.daemon) {
      const child = spawn(runtimeBin, serverArgs, {
        ...
        env: {
          ...process.env,
          IKANBAN_PORT: options.port.toString(),
          OPENCODE_BINARY: opencodeBinary,
          ...(typeof effectiveUiPassword === 'string' ? { IKANBAN_UI_PASSWORD: effectiveUiPassword } : {}),
          IKANBAN_TRY_CF_TUNNEL: options.tryCfTunnel ? 'true' : 'false',
          ...(process.env.OPENCODE_SKIP_START ? { IKANBAN_SKIP_OPENCODE_START: process.env.OPENCODE_SKIP_START } : {}),
        }
      });
      ...
      const resolvedPort = await new Promise((resolve) => {
        ...
        child.on('message', (msg) => {
          if (settled) return;
          if (msg && msg.type === 'ikanban:ready' && typeof msg.port === 'number') {
            settled = true;
      ...
      if (isProcessRunning(child.pid)) {
        ...
        console.log(`IKanban started in daemon mode on port ${resolvedPort}`);
    ...
    if (typeof effectiveUiPassword === 'string') {
      process.env.IKANBAN_UI_PASSWORD = effectiveUiPassword;
    }
    if (process.env.OPENCODE_SKIP_START) {
      process.env.IKANBAN_SKIP_OPENCODE_START = process.env.OPENCODE_SKIP_START;
    }
    if (showAutoGeneratedPassword) {
      console.log(`\n🔐 Auto-generated password: \x1b[92m${effectiveUiPassword}\x1b[0m`);
      console.log('⚠️  Save this password - it won\'t be shown again!\n');
  ...
  async stop(options) {
    ...
    try {
      ...
      const pidFiles = files.filter(file => file.startsWith('ikanban-') && file.endsWith('.pid'));

      for (const file of pidFiles) {
        const port = parseInt(file.replace('ikanban-', '').replace('.pid', ''));
        if (!isNaN(port)) {
          const pidFilePath = path.join(tmpDir, file);
          const pid = readPidFile(pidFilePath);

          if (pid && isProcessRunning(pid)) {
            const instanceFilePath = path.join(tmpDir, `ikanban-${port}.json`);
            runningInstances.push({ port, pid, pidFilePath, instanceFilePath });
          } else {

            removePidFile(pidFilePath);
            removeInstanceFile(path.join(tmpDir, `ikanban-${port}.json`));
          }
        }
      }
    } catch (error) {
    ...
    if (runningInstances.length === 0) {
      console.log('No running IKanban instances found');
      return;
    }
    ...
    if (portWasSpecified) {
      ...
      if (!targetInstance) {
        console.log(`No IKanban instance found running on port ${options.port}`);
        return;
      }

      console.log(`Stopping IKanban (PID: ${targetInstance.pid}, Port: ${targetInstance.port})...`);
      ...
      try {
        ...
        const checkShutdown = setInterval(() => {
          ...
          if (!isProcessRunning(targetInstance.pid)) {
            ...
            console.log('IKanban stopped successfully');
            ...
            removePidFile(targetInstance.pidFilePath);
            removeInstanceFile(targetInstance.instanceFilePath);
            console.log('IKanban force stopped');
          }
        }, 500);
      ...
      console.log(`Stopping all IKanban instances (${runningInstances.length} found)...`);
  ...
  async restart(options) {
    ...
    if (instancesToRestart.length === 0) {
      console.log('No running IKanban instances to restart');
      console.log('Use "ikanban serve" to start a new instance');
      return;
    }
    ...
    for (const instance of instancesToRestart) {
      console.log(`Restarting IKanban on port ${instance.port}...`);
      ...
      if (runningInstances.length > 0) {
        ...
        for (const instance of runningInstances) {
          try {
            ...
            console.log(`  Run manually: ikanban serve --port ${instance.port} --daemon`);
          }
        }
      }
    } else {
