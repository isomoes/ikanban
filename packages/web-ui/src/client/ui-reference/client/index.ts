 ...
 *
 * @module @isomoes/dsh-web-ui/client/ui-reference/client
 */
...
import { relativeTime } from '../../ui-primitives/relative-time.ts'
import type {
...
/** Register independently settling file and session `@` sources. */
export function apply(ctx: ClientContext): void {
  ...
  const fileSource: InputTriggerSource = {
    ...
    async candidates(session: ClientSessionContext, { query, quoted, drilled, signal }) {
      ...
      return result.value.flatMap(candidate => fileCandidate(candidate, quoted === true, withLocation, t))
    },
    header(_session: ClientSessionContext, req) {
      return crumbsFor(req.query, req.quoted === true, req.drilled, t)
    },
    onPick: ({ candidate, action }) => pickReference(candidate, action, FILE_SOURCE_NAME),
  ...
  const sessionSource: InputTriggerSource = {
    ...
    async candidates(session: ClientSessionContext, { query, quoted, signal }) {
      ...
      return result.value.map(candidate => sessionCandidate(
        candidate,
    ...
    onPick: ({ candidate, action }) => pickReference(candidate, action, SESSION_SOURCE_NAME),
    codec: referenceCodec,
  }
...
function pickReference(candidate: { value?: string }, action: string | undefined, source: string) {
  const value = parseCandidate(candidate.value)
  if (value?.kind === 'file') {
    if (value.fileKind === 'directory' && action === 'drill') return { text: value.mention, continue: true as const }
    return {
      insert: {
        source,
        ref: value.mention,
        label: value.fileKind === 'directory' ? `${value.label}/` : value.label,
        appearance: value.fileKind === 'directory' ? 'folder' as const : 'file' as const,
        clipboardText: value.mention,
      },
    }
  }
  if (value?.kind === 'session') {
    return { insert: { source, ref: value.mention, label: value.label, appearance: 'session' as const, clipboardText: value.mention } }
  }
  return undefined
}
