 ...
 */
export const inject = ['slots', 'locale', 'connection', 'remote', 'remote.settings', 'settingsScope', 'commandUi']

/** Keep local-document controls available on all browser loopback origins. */
function isLocalOrigin(ctx: ClientContext): boolean {
  if (ctx.remote.$host.isLoopback) return true
...
export function apply(ctx: ClientContext): void {
  ...
  const documentController = isLocalOrigin(ctx)
  ...
  const shellInjected = (): SettingsRootInjected => ({
    registerOpenAction: open => ctx.commandUi.registerAction({
      id: 'settings.open',
