...
/**
 * Host facts normally identify local persistence, but a dev server can be
 * reached as `localhost` while the remote reports a different loopback name.
 * Treat the browser's loopback origin as local as well.
 */
function isLocalOrigin(ctx: Context): boolean {
...
export function apply(ctx: Context): void {
  ...
  const persistence = isLocalOrigin(ctx) ? 'host' : 'memory'
