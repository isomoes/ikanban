...
import type {} from '@isomoes/dsh-web-ui/client/ui-workspace/client'
...
export const inject = [
  'slots', 'locale', 'remote', 'remote.agentPresets', 'remote.settings', 'commandUi',
]
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.inject(['slots', 'conversation', 'sessions', 'uiWorkspace'], (scope: ClientContext) => {
    ...
    const chooseModeAction = scope.commandUi.registerAction({
      ...
      title: () => scope.locale.bind('settings.agentPreset')('nav'),
      category: () => 'Sessions',
      keybind: 'mod+shift+m',
      ...
      run: () => {
        ...
        if (current?.blank !== true) scope.uiWorkspace.startSession()
        window.dispatchEvent(new Event('dsh-agent-preset-open'))
      },
    })
