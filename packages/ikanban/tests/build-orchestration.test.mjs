...
test('build lifecycles produce private Web UI artifacts without recursive orchestration', async () => {
  ...
  assert.equal(ikanban.dependencies['@deepseek-ai/dsh-agent-loop'], '^0.1.0-rc.7')
  assert.equal(ikanban.dependencies['@deepseek-ai/dsh-tools'], '^0.1.0-rc.7')
  ...
  assert.equal(ikanban.dependencies['@deepseek-ai/dsh-mcp-client'], '^0.1.0-rc.7')
  assert.equal(ikanban.peerDependencies['@deepseek-ai/dsh-system-prompt'], '^0.1.0-rc.7')
  assert.equal(projectMcp.private, true)
  assert.equal(projectMcp.dependencies['@deepseek-ai/dsh-mcp-client'], '^0.1.0-rc.7')
