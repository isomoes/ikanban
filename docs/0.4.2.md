...
`entries.json` 一共定义 30 个构建条目，其中 28 个直接进入主浏览器 roster；另外两个分别是原生目录选择器和浏览器目录选择器。公开包默认动态挂载浏览器目录选择器，原生版本会被构建，但不会进入默认运行组合。
...
| 浏览器客户端清单 | [`packages/web-ui/src/entries.json`](../packages/web-ui/src/entries.json) |
